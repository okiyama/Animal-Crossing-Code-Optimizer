...
else
{
	...
	//echo acucDataToHex($stepDecBitCode);
	$stepDecRSACiph = acucDecodeRSACipher( $stepDecBitCode);
	//echo acucDataToHex($stepDecRSACiph);
	/*$returnArrays = acucChangeRSACipherRange( $stepDecRSACiph);
	foreach($returnArrays as $returnArray)
	{
		echo acucDataToHex($returnArray);
	...
	$codeCheck >>= 3;
	$codeRealCheck = ($codeChecksum + 0xff) & 0x03;
