...
{
	multiVal = document.getElementById("doMulti").checked;
	multi = (multiVal ? "&multi=yes" : "&multi=no");
	stringURL = location.protocol + "//" + location.host + location.pathname;
	stringURL = replaceAll(stringURL, "index.php", "");
	
	stringURL += "index.php?mod=generator&data=" + code + multi;
...
function changeByte0()
{
	isChecked = document.getElementById("cByte0").checked;
	if( isChecked )
	{
		document.getElementById("cByte0Div").style.visibility = "visible";
	}
	else
	{
		document.getElementById("cByte0Div").style.visibility = "hidden";
	}
	updateNumber("byte0", 2);
	showFullCode();
}
...
function showFullCode()
{
	...
	if(document.getElementById("cByte0").checked)
	{
		byte0 = document.getElementById("byte0Value").value;
		checkCode = hexdec(byte0) % 4;
	}
...
function generateBlock()
{
	...
	generateXML.open("GET","./generator.output.php?data=" + sendString + multi,true);
	generateXML.send();
}
...
?>
	...
	<table border="0">
	 <tr>
	  ...
	  <td style="width: 25%;">
		<input type="checkbox" id="cByte0" onChange="changeByte0();" />
		<b>Override checksum</b>
	  </td>
	  <td style="width: 25%;">
	   <div style="width: 100%; height: 100%; visibility: hidden;" id="cByte0Div"><b>Checksum:</b><br />
		<input type="text" id="byte0" style="font-size: x-large; font-family: monospace;" size="2" maxlength="2" onclick="changeCursor(-1);" onkeyup="updateNumber('byte0', 2)" value="00" /><input type="hidden" id="byte0Value" value="00" /></div>
	  </td>
	 </tr>
	</table>
	...
	<input type="checkbox" id="doMulti" onChange="showFullCode();"> Generate multiple codes
   </td>
  </tr>
  <tr>
...
<?php
if(isset($_REQUEST["multi"]) && $_REQUEST["multi"] == "yes")
{
	echo "document.getElementById(\"doMulti\").checked=true;\n";
}
if($_REQUEST["data"])
{
