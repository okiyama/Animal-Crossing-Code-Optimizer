...
//
//////////////////////////////////////////////////////
function acucChangeRSACipherRange( $arrayCode )
{
	$primeParams = acucGetRSAKeyCode( $arrayCode );
	...
	for($idx = 0; $idx < 8; $idx++)
	{
		$valueTable[$idx] = $modBackTo[ $arrayCode[ $indexTable[ $idx ] ] ];
		if(empty($valueTable[$idx]))
		{
			$valueTable[$idx][] = $modChange[ $arrayCode[ $indexTable[ $idx ] ] ];
		}
	}
	
	$codeList = array();
	$iterator = array(0, 0, 0, 0, 0, 0, 0, 0);
	
	while( end($iterator) < sizeof(end($valueTable)))
	{
		$arrayReturnCode = $arrayCode;
		$checkByte = 0;
		for($idx = 0; $idx < 8; $idx++)
		{
			$valueByte = $valueTable[ $idx ][ $iterator[ $idx ] ];
			$arrayReturnCode[ $indexTable[ $idx ] ] = $valueByte & 0xff;
			$valueByte = ($valueByte >> 8 ) & 0x01;
			$checkByte |= $valueByte << $idx;
		}
		...
		$codeList[] = $arrayReturnCode;
		
		$iterator[0]++;
		
		$idx = 0;
		
		while($iterator[$idx] >= sizeof($valueTable[$idx]) && $idx < 7)
		{
			$iterator[$idx] = 0;
			$idx++;
			$iterator[$idx]++;
